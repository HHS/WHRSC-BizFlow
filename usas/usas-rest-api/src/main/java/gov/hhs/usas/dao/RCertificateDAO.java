...
public interface RCertificateDAO extends JpaRepository<RCertificate, String> {
	...
	@Query(value = "SELECT * FROM VW_R_CERT_RESULT WHERE REQUEST_NUMBER = ?1", nativeQuery = true)
	public List<RCertificate> findByRequestNumber(String requestNumberParam);
	
	@Query(value = "SELECT * FROM VW_R_CERT_RESULT WHERE REQUEST_NUMBER = ?1 AND VACANCY_NUMBER = ?2 AND ANNOUNCEMENT_NUMBER = ?3", nativeQuery = true)	    
