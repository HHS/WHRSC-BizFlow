...
public interface ACertificateDAO extends JpaRepository<ACertificate, String> {
	...
	@Query(value = "SELECT * FROM VW_A_CERT_RESULT_TWO WHERE REQUEST_NUMBER = ?1", nativeQuery = true)
	public ACertificate findByRequestNumber(String requestNumberParam);
	
	@Query(value = "SELECT DISTINCT * FROM VW_A_CERT_RESULT_TWO WHERE REQUEST_NUMBER = ?1 AND VACANCY_NUMBER = ?2", nativeQuery = true)	    
